...
- `mobile/`: React Native/Expo 57 + TypeScript, with pinned dependencies and lockfile; future Android shares this code. The app now lives under `mobile/` in the private `MarcosBuratovich/resbite-app` repository.
...
- TypeScript and twelve domain/secure-storage tests pass; eight draft content/artwork pairs pass the manifest check.
...
1. Retry/reconciliation across app restarts and uncertain network outcomes. Owner editing and organiser list refresh on foreground are implemented; two-device integration still needs verification.
...
## Plan editing update — 17 September 2026

- Added owner-only editing of a future active plan: date/time, meeting place and note. The original server version is sent with the edit, and the form retains the draft on failure. A stale version explicitly offers a reload before saving again.
- The organiser's plan/RSVP list refreshes when the screen gains focus, when the app returns to foreground and when pulled to refresh. Late responses cannot overwrite a newer load or update an unfocused screen. Loading failures have an explicit retry state.
- Five new domain tests passed after failing against unimplemented behavior; full check now runs twelve tests. Database rollback assertions additionally confirm a successful owner edit, unchanged accepted RSVP, rejected stale edit and a single plan-change notification queued for the attendee.
- UI verification at phone width covered create → edit place/note → save, invalid date with draft retained, and cancelled plan with edit action removed. No browser runtime errors appeared in that flow. iOS JavaScript export passed again; actual native device checks remain pending.
- GitHub repository: https://github.com/MarcosBuratovich/resbite-app (private). The Linux checkout is now the `repository/` subfolder of the original workspace. Original top-level app/doc folders are preserved snapshots. Mac users clone the GitHub repository directly and follow its README.
